"""
Tax Advisor - LLM orchestration for tax-related queries.
Coordinates between intent classification, tax calculators, AI/RAG, and NLP.

Part of services/ai/ - may import from core/ but never the other way around.
"""
...
from core.intent_classifier import ClassificationResult, Intent, IntentClassifier
...
class TaxAdvisor:
    """
    Tax Advisor: LLM-orchestrated tax consultation.

    Pipeline:
    1. Classify intent (keyword-based)
    2. Extract entities from the message
    3. Route to TaxCalculator (pure) or RAG/LLM handler
    ...
    def __init__(self, rag_service: RAGService | None = None) -> None:
        ...
        # Expose tax_rules dict for backward-compat with api/routes/tax.py
        self.tax_rules = self._calculator.rules
    ...
    async def _handle_calculation(
        ...
        if category not in self._calculator.supported_categories:
            return self._build_response(
    ...
    async def _handle_tax_info(
        ...
        if category and category in self._calculator.supported_categories:
            info = self._calculator.get_info(category, customer_type)
