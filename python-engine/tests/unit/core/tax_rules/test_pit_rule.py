...
class TestPITProgressiveBrackets:
    ...
    def test_bracket_2_full(self, pit_rule: PITRule):
        """Income 45.5 triệu → taxable = 30 triệu.
        10tr × 5% + 20tr × 10% = 500k + 2,000k = 2,500k.
        """
        ctx = TaxContext(income=45_500_000)
        result = pit_rule.calculate(ctx)

        assert result.amount == pytest.approx(2_500_000)
    ...
    def test_bracket_3_full(self, pit_rule: PITRule):
        """Income 75.5 triệu → taxable = 60 triệu.
        10×5% + 20×10% + 30×20% = 500k + 2,000k + 6,000k = 8,500k.
        """
        ctx = TaxContext(income=75_500_000)
        result = pit_rule.calculate(ctx)

        assert result.amount == pytest.approx(8_500_000)
    ...
    def test_bracket_4_full(self, pit_rule: PITRule):
        """Income 115.5 triệu → taxable = 100 triệu.
        10×5% + 20×10% + 30×20% + 40×30%
        = 500k + 2,000k + 6,000k + 12,000k = 20,500k.
        """
        ctx = TaxContext(income=115_500_000)
        result = pit_rule.calculate(ctx)

        assert result.amount == pytest.approx(20_500_000)
